...
class wave2d():
    def __init__(self, xNodes, yNodes, dx, dy, dt, speed):
        ...
        self.obstacle  = np.ones(shape=(xNodes,yNodes), dtype="float64")            #Obstacle inside the domain.
    ...
    def setObstacle(self, obstacle):
        if self.field[0,:,:].shape == obstacle.shape and obstacle.dtype == "float64":  #The obstacle must be the same shape and type as the field for this to work
    ...
    def setScreenAt(self, size, xPosition, yPosition, alignment=0):
        ...
        self.screenXpos = int((self.xNodes - 1)*xPosition)     #Set the 'x' position of the screen center, xPosition is relative to the total width of the domain [0,1]
        self.screenYpos = int((self.yNodes - 1)*yPosition)     #Set the 'y' position of the screen center, yPosition is relative to the total height of the domain [0,1]
    ...
    def getAbsoluteScreen(self):
        if self.screenOn == True:   #If the screen is in use return its value depending on alignment
            ...
            return self.absoluteScreen.copy()   #Retutn a copy of the screen


    def getWave(self):
        wave = self.field[2,:,:].copy()     #Auxiliar variable, use for not destructive visualization of the field
